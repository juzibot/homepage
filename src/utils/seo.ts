export function getPageKeywords(pathname?: string) {
  const map: { [p: string]: string } = {
    ...
    'features/case': 'ChatGPT、人工智能、企业管理、知识管理、RPA、AI客服、人机协作、IM跨平台、企微SCRM、私域客服、句子互动、企微管家、私域运营、私域工具、私域流量、客户管理、精细化运营、批量加好友、聚合聊天、消费品行业、互联网行业、人工智能、ChatGPT、AIGC、AI、腾讯广告、德伦口腔、九十里',
  };
  if (!pathname) {
    return map['/'];
  }
...
export function getPageDescription(pathname?: string) {
  const map: { [p: string]: string } = {
    ...
    'features/case': '句子互动是一家人工智能技术与服务提供商，主要提供基于社交生态的智能机器人营销云系统。句子互动的主营业务包括销售、市场和售后，通过智能对话、客服系统和数据分析帮助企业快速获客、增加用户粘性并提高转化。句子互动的产品和服务涵盖了多个场景，包括微信、WhatsApp等社交平台，以及教育、消费品、金融保险、政务服务等领域。服务了腾讯广告、德伦口腔、九十里等客户。',
  };
  if (!pathname) {
    return map['/'];
  }
